...
// Embedded 8x16 font from the PC BIOS/VGA set, covering printable ASCII 32–126.
...
// Drawing helpers.
// These coordinates are relative to the terminal area, and the offsets are added when drawing to the framebuffer.

static void draw_cursor(int px, int py, uint32_t colour)
{
    // Draw a small block cursor at the bottom of the character cell.
...
{
    _bg = fb_colour(r, g, b);
    /* Clear just the terminal area instead of the whole screen. */
    clear_terminal_area();
    _cx = PADDING;
    _cy = PADDING;
}
...
{
    // Remove the old cursor before drawing the next character.
    if (_cursor_visible)
    {
