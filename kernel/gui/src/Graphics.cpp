...
// Same 8x16 VGA-style font used by the terminal (printable ASCII 32-126)
static const uint8_t font[95][16] = {
    ...
    {0x00, 0x00, 0x7F, 0x60, 0x60, 0x60, 0x7E, 0x03, 0x03, 0x03, 0x63, 0x3E, 0x00, 0x00, 0x00, 0x00}, // '5'
    ...
    {0x00, 0x00, 0x1E, 0x33, 0x61, 0x60, 0x60, 0x6F, 0x63, 0x63, 0x33, 0x1E, 0x00, 0x00, 0x00, 0x00}, // 'G'
    ...
    {0x00, 0x00, 0x3E, 0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x6B, 0x6F, 0x3E, 0x06, 0x03, 0x00, 0x00}, // 'Q'
    ...
    {0x00, 0x00, 0x63, 0x63, 0x63, 0x63, 0x63, 0x6B, 0x6B, 0x7F, 0x77, 0x63, 0x00, 0x00, 0x00, 0x00}, // 'W'
    {0x00, 0x00, 0x63, 0x63, 0x36, 0x36, 0x1C, 0x1C, 0x36, 0x36, 0x63, 0x63, 0x00, 0x00, 0x00, 0x00}, // 'X'
    {0x00, 0x00, 0x63, 0x63, 0x63, 0x36, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 0x3E, 0x00, 0x00, 0x00, 0x00}, // 'Y'
    ...
    {0x00, 0x00, 0x0C, 0x0C, 0x00, 0x1C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x3F, 0x00, 0x00, 0x00, 0x00}, // 'i'
    {0x00, 0x00, 0x06, 0x06, 0x00, 0x0E, 0x06, 0x06, 0x06, 0x06, 0x06, 0x66, 0x66, 0x3C, 0x00, 0x00}, // 'j'
    {0x00, 0x00, 0x70, 0x30, 0x30, 0x33, 0x36, 0x3C, 0x36, 0x33, 0x33, 0x73, 0x00, 0x00, 0x00, 0x00}, // 'k'
    {0x00, 0x00, 0x1C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x3F, 0x00, 0x00, 0x00, 0x00}, // 'l'
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x66, 0x7F, 0x6B, 0x6B, 0x63, 0x63, 0x63, 0x00, 0x00, 0x00, 0x00}, // 'm'
    ...
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x3E, 0x63, 0x30, 0x1C, 0x06, 0x63, 0x3E, 0x00, 0x00, 0x00, 0x00}, // 's'
...
{
    fb_ = fb;
    fb_init(fb); // safe even if already called
}

void Graphics::clear(uint32_t colour)
{
    fb_clear(colour);
}

void Graphics::fill_rect(int x, int y, int w, int h, uint32_t colour)
{
    if (w <= 0 || h <= 0)
        return;
...
void Graphics::draw_rect(int x, int y, int w, int h, uint32_t colour)
{
    if (w <= 0 || h <= 0)
        return;
...
void Graphics::put_pixel(int x, int y, uint32_t colour)
{
    fb_putpixel((uint64_t)x, (uint64_t)y, colour);
}

uint32_t Graphics::colour(uint8_t r, uint8_t g, uint8_t b)
{
...
{
    return fb_ ? (int)fb_->width : 0;
}

int Graphics::height() const
{
    return fb_ ? (int)fb_->height : 0;
}

void Graphics::draw_char(int x, int y, char c, uint32_t fg, uint32_t bg)
{
    if (c < 32 || c > 126)
        c = '?';
...
void Graphics::draw_string(int x, int y, const char *str, uint32_t fg, uint32_t bg)
{
    if (!str)
        return;
    int cx = x;
